...
<script type="text/javascript">
	...
	function newRow(rowNum, pro_name, pro_id, option_id, unit){
		...
		var row = '<div class="row copied pB15 mB15 borderB" row="'+rowNum+'" style="margin-bottom:10px;">\
					...
					<div class="col-sm-2 cc">\
						<input type="number" step="0.001" name="purchase_price[]" class="xcopy calc form-control price_'+rowNum+'" relType="price" rel="'+rowNum+'" placeholder="0.000"/>\
					</div>\
					<div class="col-sm-1 cc">\
	...
	$(document).on("keyup change", ".calc", function(){
		...
		var rowTotal = parseFloat(qty * price).toFixed(3);
		
		var taxTotal = parseFloat( ( rowTotal / 100) * tax  ).toFixed(3);
		
		var lastRowTotal = parseFloat(rowTotal + taxTotal).toFixed(3);
		//alert(lastRowTotal);
		console.log("qty : " + qty);
	...
	/* $(document).on("keyup", ".calcT", function(){
		...
		var unitPrice = parseFloat(total / qty).toFixed(3) ;
