...
<div class="table">
	<div class="row" style="margin:0;">
		<div class="col-sm-1 autoHeight leftDiv" style="padding:0;"> 
			<div>
				<a href="javascript:;"  class="leftBt" style="background:#3D3D3D;"></a>
				<a href="javascript:;"  class="leftBt" style="background:#3D3D3D;"></a>
				<a href="javascript:;"  class="leftBt" style="background:#3D3D3D;"></a>
				<a href="javascript:;"  class="leftBt" style="background:#3D3D3D;"></a>
				...
				<?php if(count($printers) == 1){ ?>
						...
						<a href="javascript:;" class="leftBt" style="background:#3D3D3D;"><!--printX-->
		...
		<div id="printDiv" class="col-sm-5 autoHeight" style="padding:0;padding-right:3px; border-right:1px solid #4b4b4b;">
			...
			<div class="xtab adisyonDiv">
				...
				<div class="payBottom">
						...
						<input type="hidden" class="ttPP" value="<?php echo $last_order['rest_price'];?>" />
									...
									<td style="padding:9px 2px !important">
										<span style="display:inline-block; float:left;">
											<?php if($_SESSION['user_type_id'] != '4'){ ?>
												<?php if($table['is_busy'] > 0){ ?>
													<a href="javascript:;" table_id="<?php echo $table['table_id'];?>" class="btn btn-2 btn-2c btn-sm btn-float " data-toggle="modal" data-target="#myModal" style="font-size:15px;background:#000;"><b>PARÇALI</b></a>
													
													<a href="javascript:;" table_id="<?php echo $table['table_id'];?>" class="btn btn-2 btn-2c btn-sm btn-float payCC" style="font-size:15px;background:#5ec76a;"><b>K. KARTI</b></a>
													<a href="javascript:;" table_id="<?php echo $table['table_id'];?>" class="btn btn-2 btn-2c btn-sm btn-float payCash" style="font-size:15px;background:#53c3dc;"><b>NAKİT</b></a>
		...
		<!--<div class="col-sm-2 autoHeight hidden-xs" style="padding:0; padding-right:3px;  overflow-y:auto; background:#fdfdfd;">
			<?php foreach($cats as $key => $val){ ?> 
				<div class="">
		...
		<div class="col-sm-6 autoHeight hidden-xs removeXs" style="padding:0; background:#f5f5f5; overflow-y:auto; background:#6d6b6b; ">
			<div class="<?php if($kiosk_order == '1'){ echo 'hidden'; } ?>">
...
function printKitchen() 
{
  ...
  //location.reload();
	...
	$(".payCash").click(function(){
		...
		var amount = parseFloat($(".ttPP").val()).toFixed(2);
		var table_id = $(this).attr("table_id");
		$.ajax({
			type : "post",
			url : "<?php echo ORDER_PAYMENT_POST;?>",
			data : {"amount" : amount, "table_id" : "0", "order_id" : order_id, "payment_type" : "cash", "sub_p_id" : "",  "customer_id" : "0" },
			success : function(response){
				if(response == 'success'){
					$("#successDiv").show();
	...
	$(".payCC").click(function(){
		...
		$.ajax({
			type : "post",
			url : "<?php echo ORDER_PAYMENT_POST;?>",
			data : {"amount" : amount, "table_id" : "0", "order_id" : order_id, "payment_type" : "credit", "sub_p_id" : "",  "customer_id" : "0" },
			success : function(response){
				if(response == 'success'){
					$("#successDiv").show();
					$("#successDiv").fadeOut(500);
						...
						<div class="modal-dialog" style="position:relative;">\
						  <div class="modal-content">\
							...
							<div class="text-center"><b>İptal veya İkram etmek istediğiniz adeti yazınız.</b></div>\
							  <?php if($ikr == '1'){ ?>\
								<p class="'+ikr+'" style="margin-bottom:15px;">\
									  <div class="row '+ikr+'">\
									  <div class="col-xs-6">\
