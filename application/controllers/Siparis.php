...
class Siparis extends CI_Controller {
	...
	public function main_board(){
		...
		$this->load->view('main_board_view');
	...
	public function adisyon_group_post(){
		$post = $this->input->post();
		
		if(!empty($post['adisyon_group'])){
		...
		$this->db->update("settings_table", array('adisyon_group' => $gr), array('id' => '1'));
	...
	public function cost_list(){
		$data['title'] = 'Reçeteli Maliyet Ürün Listesi';
		$data['list'] = $this->db->select('products_table2.pro_id, pro_name')
			->join('porsions_table', 'recipes_table.porsion_id = porsions_table.id', 'left')
			->join('products_table2', 'porsions_table.pro_id = products_table2.pro_id', 'left')
			->group_by('porsion_id')
		...
		$data['mm'] = '16';
		$data['mt2'] = '16_4';
		$this->load->view('cost_list_view', $data);
		
	}
	
	public function update_recipe_cost(){
		
		$data['list'] = $this->db->select('porsion_id,products_table2.pro_id, pro_name')
			->join('porsions_table', 'recipes_table.porsion_id = porsions_table.id', 'left')
			->join('products_table2', 'porsions_table.pro_id = products_table2.pro_id', 'left')
			->group_by('porsion_id')
			->get('recipes_table')->result_array();
		...
		foreach($data['list'] as $key => $val){
			$data['list'][$key]['products'] = $this->db->select('*')
	...
	public function calculate_recipe_cost($porsion_id){
		
	}
	
	public function print_pro_cost($porsion_id){
		
		require_once DOC_ROOT . 'phpexcel/Classes/PHPExcel.php';
		
		$pors = $this->db->select('products_table2.pro_id, porsion_price, pro_name, porsion_name')
			->join('products_table2', 'porsions_table.pro_id = products_table2.pro_id', 'left')
			->where('id', $porsion_id)
			->get('porsions_table')->row_array();
		
		$recipe = $this->db->select('*')
			->where('porsion_id', $porsion_id)
			->get('recipes_table')->result_array();
		
		foreach($recipe as $kk => $vv){
			$recipe[$kk]['cost'] = $this->get_cost($vv['c_pro_id']);
			$recipe[$kk]['pro_name'] = $this->get_pro_name($vv['c_pro_id']);
		}
		//debug($recipe);
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A1', $pors['pro_name'].' '.$pors['porsion_name']);
		$objPHPExcel->getActiveSheet()
					->setCellValue('A3', 'Ürün')
					->setCellValue('B3', 'Miktar')
					->setCellValue('C3', 'Fire')
					->setCellValue('D3', 'Fire Dahil M.')
					->setCellValue('E3', 'Birim')
					->setCellValue('F3', 'Birim Fiyat')
					->setCellValue('G3', 'Tutar')
					;
		...
		for($i=0; $i<count($recipe); $i++){
			$ii = $i+4;
			$objPHPExcel->getActiveSheet()->setCellValue('A'.$ii, $recipe[$i]['pro_name']);
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$ii, $recipe[$i]['qty']);
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$ii, $recipe[$i]['loss']);
			$objPHPExcel->getActiveSheet()->setCellValue('D'.$ii, ($recipe[$i]['qty'] + (($recipe[$i]['qty']/100)*$recipe[$i]['loss']) ) );
			$objPHPExcel->getActiveSheet()->setCellValue('E'.$ii, $recipe[$i]['unit']);
			$objPHPExcel->getActiveSheet()->setCellValue('F'.$ii, $recipe[$i]['cost']);
			$objPHPExcel->getActiveSheet()->setCellValue('G'.$ii, ( $recipe[$i]['cost'] * ($recipe[$i]['qty'] + (($recipe[$i]['qty']/100)*$recipe[$i]['loss']) ) ) );
			$tot += ( $recipe[$i]['cost'] * ($recipe[$i]['qty'] + (($recipe[$i]['qty']/100)*$recipe[$i]['loss']) ) );
			$unit[$recipe[$i]['unit']] += $recipe[$i]['qty'];
		}
		$ix = $ii;
		$ii = $ii+1;
		...
		$objPHPExcel->getActiveSheet()
			->setCellValue('A'.$ii, 'Kalem Adedi')
			->setCellValue('B'.$ii, count($recipe))
			->setCellValue('C'.$ii, '')
			->setCellValue('D'.$ii, '')
			->setCellValue('E'.$ii, '')
			->setCellValue('F'.$ii, 'Toplam')
			->setCellValue('G'.$ii, $tot)
			;
		
		
		$objPHPExcel->getActiveSheet()->getStyle("A".$ii.":G".$ii)->getFill()->applyFromArray(array(
		...
		$objPHPExcel->getActiveSheet()->getStyle('A1:G1')->getFont()->setSize(22);
			
		$ii = $ii+1;
		$i3 = $ii+2;
		
		$objPHPExcel->getActiveSheet()->mergeCells('A'.$ii.':A'.$i3);
		$objPHPExcel->getActiveSheet()->setCellValue('A'.$ii, 'Birim Toplamları');
		
		$objPHPExcel->getActiveSheet()->getStyle("A".$ii.":G".$i3)->getFill()->applyFromArray(array(
			'type' => PHPExcel_Style_Fill::FILL_SOLID,
			'startcolor' => array(
				 'rgb' => 'F1EEAB'
			)
		...
		foreach($unit as $key => $val){
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$ii, strtoupper(substr($key,0,2)) );
			$objPHPExcel->getActiveSheet()->setCellValue('C'.$ii, $val );
			$ii++;
		}
		...
		$objPHPExcel->getActiveSheet()->setCellValue('A2', 'Porsiyon 1 Ad.');
		$objPHPExcel->getActiveSheet()->setCellValue('C2', 'Net Satış Fiyatı :');
		$objPHPExcel->getActiveSheet()->setCellValue('D2', $pors['porsion_price']);
		$objPHPExcel->getActiveSheet()->setCellValue('F2', 'Birim Maliyet :');
		$objPHPExcel->getActiveSheet()->setCellValue('G2', $tot);
		
		$ix4 = $ix+4;
		// Rename worksheet
		$objPHPExcel->getActiveSheet()->setTitle('Maliyet Reçetesi');
		foreach(range('A','H') as $columnID) {
			$objPHPExcel->getActiveSheet()->getColumnDimension($columnID)->setAutoSize(true);
		}
		$objPHPExcel->getActiveSheet()->getStyle("A1:G3")->getFont()->setBold( true );
		$objPHPExcel->getActiveSheet()->getStyle("A".$ix.":G".$ix4)->getFont()->setBold( true );
	...
	public function fix_menu_post(){
		$post = $this->input->post();
		
		if(!empty($post['fix_menu'])){
		...
		$this->db->update("settings_table", array('fix_menu' => $gr), array('id' => '1'));
		
		redirect($_SERVER['HTTP_REFERER']);
		
	}
	
	public function wait_product_post(){
		$post = $this->input->post();
		
		if(!empty($post['wait_product'])){
		...
		$this->db->update("settings_table", array('wait_product' => $gr), array('id' => '1'));
	...
	public function print_reservation(){
		...
		if(!empty($post['printer_type'])){
			$ins['printer_type'] = $post['printer_type'];
		}
		$ins['insertTime'] = time();
		...
		$row[] = array('print_type' => 'receipt');
		$row[] = array('type' => 'header', 'title' => $date);
		$row[] = array('type' => 'header', 'title' => 'Rezervasyon Listesi');
		//$row[] = array('type' => 'header', 'title' => $arr['user_name']);
		$row[] = array('type' => 'tableCell', 'cell1' => 'Saat', 'cell2' => 'isim','cell3' => 'Kisi',  'cell4' => 'Not');
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		
		foreach( $list as $key => $val ){
			
				$row[] = array('type' => 'tableCell', 'cell1' => $val['reservation_hour'], 'cell2' => $val['full_name'], 'cell3' => $val['total_person'],  'cell4' => $val['note'] );
	...
	public function product_cost($pro_id){
		
		$porsions = $this->db->select('*')
			->where('pro_id', $pro_id)
			->get('porsions_table')->result_array();
		
		foreach($porsions as $key => $val){
			$porsions[$key]['pro_name'] = $this->get_pro_name($val['pro_id']);
			//$porsions[$key]['pro_price'] = $this->get_pro_name($val['pro_id']);
			$porsions[$key]['recipe'] = $this->db->select('*')
				->where('porsion_id', $val['id'])
				->get('recipes_table')->result_array();
			
			foreach($porsions[$key]['recipe'] as $kk => $vv){
				$porsions[$key]['recipe'][$kk]['cost'] = $this->get_cost($vv['c_pro_id']);
				$porsions[$key]['recipe'][$kk]['pro_name'] = $this->get_pro_name($vv['c_pro_id']);
			}
			
		}
		
		$data['porsions'] = $porsions;
		$data['mm'] = '16';
		$data['mt2'] = '16_4';
		//debug($data['porsions']);
		$this->load->view('product_cost_view', $data);
		
	}
	
	public function update_costs(){
		$list = $this->db->select('pro_id, SUM(qty) qty, SUM(total_price) total_price')
			->group_by('pro_id')
			->get('purchase_details_table')->result_array();
		
		$query = 'SELECT pro_id, MAX(row_insert_time) as time
					FROM purchase_details_table
					GROUP BY pro_id';
		
		$list2 = $this->db->query($query)->result_array();
		
		
		foreach($list2 as $key => $val){
			$list2[$key] = $this->db->select('pro_id, purchase_price')
				->where('row_insert_time', $val['time'])
				->get('purchase_details_table')->row_array();
		}
		
		foreach($list as $key => $val){
			$whr[$key]['pro_id'] = $val['pro_id'];
			$upd[$key]['average_price'] = number_format(($val['total_price'] / $val['qty']),2);
				$this->db->update('porsions_table', $upd[$key], $whr[$key]);
		}
		
		foreach($list2 as $key => $val){
			$whr[$key]['pro_id'] = $val['pro_id'];
			$upd[$key]['last_price'] = $val['purchase_price'];
				$this->db->update('porsions_table', $upd[$key], $whr[$key]);
		}
	...
	public function get_cost($pro_id){
		...
		if(empty($check)){
			$price = $this->db->select('average_price, last_price')
				->where('pro_id', $pro_id)
				->order_by('id', 'DESC')
				->get('porsions_table')->row_array();
			...
			foreach($check as $key => $val){
				$price[$key] = $this->db->select('average_price, last_price')
					->where('pro_id', $val['c_pro_id'])
					->get('porsions_table')->row_array();
				$totP += number_format(($val['qty']*$price[$key]['average_price']),2);
			}
			return $totP;
	...
	public function get_pro_name($pro_id){
		$price = $this->db->select('pro_name')
			->where('pro_id', $pro_id)
			->get('products_table2')->row_array();
		return $price['pro_name'];
	}
	...
	public function cash_process(){
		$data['mm'] = '13';
		$data['mt'] = '13_1';
		$data['lastDay'] = $this->db->select('day_id')
	...
	public function open_day_details($day_id){
		...
		$data['titleText'] = date('d/m/Y H:i', $fdate).' - '.date('d/m/Y H:i', $ldate).' Tarihleri Arası Gün sonu Raporu';
		$data['list'] = $this->db->select('*')
		...
		$data['proList'] = $this->db->select('*, SUM(qty) AS count, SUM(total_price) AS tot_price')
				->join('porsions_table', 'order_details_table.porsion_id = porsions_table.id', 'left')
				->where("order_id IN (".$idL.")")
				->where("price > 0")
				->group_by('porsion_id')
	...
	public function open_day_details_x($day_id){
		...
		$data['proList'] = $this->db->select('*, SUM(qty) AS count')
				//->join('porsions_table', 'order_details_table.porsion_id = porsions_table.id', 'left')
				->where("order_id IN (".$idL.")")
				->group_by('porsion_id')
				->get('order_details_table')->result_array();
	...
	public function month_details($fdate, $ldate){
		...
		$data['day']['day_end_time'] = $ldate;
		
		$data['dayList'] = $this->db->select('*')
			->where('day_start_time >=', $fdate)
			->where('day_start_time <=', $ldate)
			->where('day_end_time !=', '0')
			->get('days_table')->result_array();
		
		$countDays = count($data['dayList']);
		
		
		$fdate = $data['dayList'][0]['day_start_time'];
		$ldate = $data['dayList'][$countDays-1]['day_end_time'];
		
		if( ($fdate != NULL) AND ( $ldate != NULL ) ){
	...
	public function month_details_simple($fdate, $ldate){
		...
		$data['titleText'] = date('d/m/Y H:i', $fdate).' - '.date('d/m/Y H:i', $ldate).' Tarihleri Arası Özet Raporu';
		...
		$data['proList'] = $this->db->select('pro_name, gr, price, SUM(qty) AS count, SUM(total_price) AS tot_price')
				->join('porsions_table', 'order_details_table.porsion_id = porsions_table.id', 'left')
				->where("order_id IN (".$idL.")")
				->where("price > 0")
				->group_by('porsion_id')
		...
		$this->load->view('month_details_simple_view', $data);
	}
	...
	public function day_end_post($day_id){
		...
		//$export = $this->export_sales($day_id);
		...
		if(empty($open_tables)){
			...
			echo 'Açık Masalar bulunmaktadır! <br /> Lütfen önce açık olan masaları kapatınız.';
		}
			
		
	}
	
	public function export_sales($day_id){

		$json_data['store_id'] = 'sirinevler';
		$store_id = 'sirinevler';
		...
		$start_time = $day['day_start_time'];
		$end_time = $day['day_end_time'];
		
		$json_data['day']['day_start_time'] = $start_time;
		$json_data['day']['day_end_time'] = $end_time;
		
		$orders = $this->db->select('order_id')
			->where('order_insert_time >', $start_time)
			->where('order_insert_time <', $end_time)
			->get('orders_table')->result_array();
		...
		if(!empty($order_ids)){
			...
			$qry = "SELECT porsion_id, pro_name, SUM(total_price) AS total_p, SUM(qty) AS total_qty, gr*SUM(qty) AS total_kg
				FROM order_details_table 
				JOIN porsions_table ON order_details_table.porsion_id = porsions_table.id
				WHERE order_details_table.total_price > 0 
				AND order_details_table.order_id IN ( $order_ids ) GROUP BY order_details_table.porsion_id ";
			
			$json_data['order_details'] = $this->db->query($qry)->result_array();
			
			$qry2 = "SELECT payment_type, SUM(paid_price) as total_paid FROM order_payments_table
				WHERE order_id IN ( $order_ids ) GROUP BY payment_type ";
			
			$json_data['order_payments'] = $this->db->query($qry2)->result_array();
		}else{
			die("no data");
		}
		
		$url = 'https://www.bulutkasa.com/connection_test.php';
		...
		$ins1['day_start_time'] = $start_time;
		$ins1['day_end_time'] = $end_time;
		$ins1['store_id'] = $store_id;
		$ins1['day_report'] = $json_data;
		$ins1['exported'] = '0';
		$ins1['day_report'] = $json_data;
		
		$check = $this->db->select('*')
			->where('day_start_time', $start_time)
			->get('aaaa_reports_table')->row_array();
			
		if(empty($check)){
			
			$this->db->insert('aaaa_reports_table', $ins1);
	...
	public function export_sales_multi(){
		
		$list = $this->db->select('*')
			->where('exported', '0')
			->get('aaaa_reports_table')->result_array();
		...
		if(!empty($list)){
			...
			if($connection == 'ok'){
				...
				$ch = curl_init('https://www.bulutkasa.com/kurulum/api_mrft_1/welcome/import_sales_mrft_multi');                                                                    
				...
				if($result == 'success'){
					foreach($list as $key => $val){
						$this->db->update('aaaa_reports_table', array('exported' => '1'), array('id' => $val['id']));
					}
				}
			}
	...
	public function prrr(){
		...
		$list = $this->db->select('pro_id,porsion_name, porsion_price')
			->where('pro_id IN ( 142, 143, 144, 145, 146 )')
			->get('porsions_table')->result_array();
		...
		foreach($list as $key => $val){
			...
			$list[$key]['porsion_2_price'] = (($val['porsion_price'])/2) + 5;
			 */
			 
			/*$list[$key]['porsion_3'] = '1.5';
			$list[$key]['porsion_3_price'] = ((($val['porsion_price'])/2)*3) + 5;*/
	...
	public function table($table_id){
		...
		if($data['table']['is_busy'] > 0){
			...
			if($data['last_order']['customer_id'] > 0){
				...
				$cid = $data['last_order']['customer_id'];
				
				$data['point'] = $this->calcPoint($data['last_order']['customer_id']);
		...
		if($data['last_order']['customer_id'] > 0){
			...
			if($data['customer']['customer_group_id'] > 0){
				$data['group'] = $this->db->select('*')
					->where('id', $data['customer']['customer_group_id'])
					->get('customer_groups_table')->row_array();
			}
	...
	public function kiosk_order($table_id){
		//$this->output->enable_profiler(TRUE);
	...
	public function send_to_printer_pack(){
		$post = $this->input->post();
		//debug($post);
		...
		$customer = $this->db->select('*')
			->join('customer_addresses_table','orders_table.address_id = customer_addresses_table.id','left')
			->join('customers_table','orders_table.customer_id = customers_table.customer_id','left')
			->join('city','customer_addresses_table.city_id = city.CityID','left')
			->join('town','customer_addresses_table.town_id = town.TownID','left')
			->join('district','customer_addresses_table.district_id = district.DistrictID','left')
			->where('order_id', $post['order_id'])
			->where('customers_table.customer_id', $post['customer_id'])
			->where('customer_addresses_table.id', $post['address_id'])
			->get('orders_table')->row_array();
		//debug($customer);
		...
		$paid_price = $this->getPaidPrice($post['order_id']);
		
		$order = $this->db->select('total_price, discount_price,extra_price, paid_price, rest_price, order_insert_time, table_id')
			->where('order_id', $post['order_id'])
			->get('orders_table')->row_array();
			
		$arr['order_details'] = $this->db->select('pro_id,pro_name, qty, price, description')
			->where('order_id', $post['order_id'])
			->order_by('id', 'ASC')
			->get('order_details_table')->result_array(); 
		...
		$arr['paid_price'] = $paid_price;
		$arr['rest_price'] = number_format(($order['total_price'] - $paid_price),2);
		$arr['order_insert_time'] = date("d.m.Y H:i",$order['order_insert_time']);
		
		$row[] = array('print_type' => 'receipt');
		$row[] = array('type' => 'header', 'title' => $arr['restaurant_name']);
		$row[] = array('type' => 'header', 'title' => 'Paket Siparis');
		$row[] = array('type' => 'header', 'title' => date('d.m.Y H:i', time()));
		$row[] = array('type' => 'line', 'title' => '');
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		$row[] = array('type' => 'line', 'title' => 'Müşteri : '. $customer['full_name']  );
		$row[] = array('type' => 'line', 'title' => 'Tel : '. $customer['phone']  );
		$row[] = array('type' => 'line', 'title' => 'Adres : '. $customer['address'] );
		$row[] = array('type' => 'line', 'title' => $customer['DistrictName'] .' / '. $customer['TownName'] .' / '. $customer['CityName'] );
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		$row[] = array('type' => 'tableCell', 'cell1' => '', 'cell2' => 'Adet','cell3' => '',  'cell4' => 'Tutar');
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		
		foreach( $arr['order_details'] as $key => $val ){
			$row[] = array('type' => 'tableCell', 'cell1' => $val['pro_name'].' '.$val['description'], 'cell2' => $val['qty'],'cell3' => '',  'cell4' => number_format(($val['qty'] * $val['price']),2).' ₺');
		}
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		$row[] = array('type' => 'total', 'title' => 'Ödeme Tipi', 'value' => $this->getPaymentType($customer['payment_type']));
		$row[] = array('type' => 'total', 'title' => 'G.Toplam', 'value' => $arr['rest_price']);
	...
	public function getPaymentType($type){
		...
		if($type == 'cc'){return 'K.Karti';}
		if($type == 'meal'){return 'Yemek Karti';}
	}
	...
	public function send_to_printer(){
		...
		if($settings['adisyon_group'] == '1'){
			$arr['order_details'] = $this->db->select('pro_id, porsion_id, pro_name, SUM(qty) qty , SUM(price) price, description')
				->where('order_id', $post['order_id'])
				->order_by('id', 'ASC')
				->group_by('porsion_id')
				->get('order_details_table')->result_array(); 
		...
		*/
		...
		$row[] = array('print_type' => 'receipt');
		$row[] = array('type' => 'header', 'title' => $arr['restaurant_name']);
		$row[] = array('type' => 'header', 'title' => $arr['table_name']);
		//$row[] = array('type' => 'header', 'title' => $arr['user_name']);
		$row[] = array('type' => 'header', 'title' => date('d.m.Y H:i', time()));
		$row[] = array('type' => 'line', 'title' => '');
		$row[] = array('type' => 'tableCell', 'cell1' => '', 'cell2' => 'Adet','cell3' => '',  'cell4' => 'Tutar');
		...
		foreach( $arr['order_details'] as $key => $val ){
			if($val['description'] != 'İptal'){
				$row[] = array('type' => 'tableCell', 'cell1' => $val['pro_name'].' '.$val['description'], 'cell2' => $val['qty'],'cell3' => '',  'cell4' => number_format(($val['qty'] * $val['price']),2).' ₺');
				//$row[] = array('type' => 'productCell', 'cell1' => $val['pro_name'].' '.$val['description'], 'cell2' => $val['qty'], 'cell3' => number_format(($val['price']),2));
			}
		}
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		$row[] = array('type' => 'bold_row', 'title' => 'Toplam', 'value' => $arr['total_price']);
		if($arr['paid_price'] > 0){
			$row[] = array('type' => 'row', 'title' => 'Ödenen', 'value' => $arr['paid_price']);
		}
		if($arr['discount_price'] > 0){
			$row[] = array('type' => 'row', 'title' => 'İndirim', 'value' => $arr['discount_price']);
		}
		$row[] = array('type' => 'total', 'title' => 'G.Toplam', 'value' => $arr['rest_price']);
			...
			if($val['description'] != 'İptal'){
				...
				if($val['description'] == 'FD'){
					...
					$ins['htmlBody'] .= '<div style="font-size:25px;">
											<div class="w60">'.$val['pro_name'].' '.$ddesc[$key].' '.$this->getProDesc($val['pro_id']).'</div>
	...
	public function send_to_printer_receipt(){
		$post = $this->input->post();
		//debug($post);
		...
		foreach( $arr['order_details'] as $key => $val ){
			$row[] = array('type' => 'row', 'title' => $val['qty'].' x '.$val['pro_name'], 'value' => number_format(($val['qty'] * $val['price']),2));
			$taxRatio = $this->getTaxRatio($val['pro_id']);
			
			if($val['price'] > 0){
				if($taxRatio > 0){
					
					$taxArr[$taxRatio]['price'] += ($val['qty'] * $val['price']);
					$taxArr[$taxRatio]['kdv'] += $this->getTax( $taxRatio, $taxArr[$taxRatio]['price'] );
				}
			}
		}
		 $row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		...
		foreach($taxArr as $key => $val){
			$row[] = array('type' => 'row', 'title' => '%'.$key.' KDV SATISLAR', 'value' => number_format($val['price'],2));
			$row[] = array('type' => 'row', 'title' => 'KDV '.$key.'%', 'value' => number_format($val['kdv'],2));
			$totalTaxx += $val['kdv'];
		}
		
		$row[] = array('type' => 'line', 'title' => '-----------------------------------------------');
		
		$row[] = array('type' => 'total', 'title' => 'KDV', 'value' => $totalTaxx);
		$row[] = array('type' => 'total', 'title' => 'G.Toplam', 'value' => $arr['total_price']);
	...
	public function getTaxRatio($pro_id){
		$pro = $this->db->select('kdv')
			->where('pro_id', $pro_id)
			->get('products_table2')->row_array();
		return $pro['kdv'];
	}
	
	public function getTax($ratio, $price){
		
		$tax = number_format((($price / ( 100 + $ratio )) * $ratio),2);
		
		return $tax;
		
	}
	
	public function getPaidPrice($order_id){
		$paid_price = $this->db->query("SELECT SUM(paid_price) total
									FROM
										order_payments_table
									WHERE
										order_id = $order_id ")->row_array();
		return $paid_price['total'];
	}
	
	public function getProDesc($pro_id){
		$pro = $this->db->select('pro_description')
			->where('pro_id', $pro_id)
			->get('products_table2')->row_array();
		return $pro['pro_description'];
	}
	...
	public function send_to_printer_report(){
		...
		if(!empty($post['reportDetail'])){
			$ins['jsonArray'] = $post['reportDetail'];
		}
		...
		print_report($post['reportDetail'], 'marifet');
	...
	public function close_order(){
		...
		if($this->db->affected_rows() > 0){
			...
			if($order['customer_id'] > 0){
				$group_point = $this->getCustomerDiscount($order['customer_id']);
			
				$ins11['point'] = number_format( ($order['total_price'] * ( $group_point / 100 )), 2 );
				$ins11['customer_id'] = $order['customer_id'];
				$ins11['order_id'] = $post['order_id'];
				$ins11['type'] = 'plus';
				$ins11['insert_time'] = time();
					$this->db->insert("customer_point_table",$ins11);
			...
			$this->db->delete('order_details_table', array('order_id' => $order_id));
				...
				//$this->db->truncate("aaa_print_table");	
					...
					foreach( $kitchen as $key => $val ){
						...
						$ins[$key]['printer_type'] = $this->getPrinterType($key); 
						...
						$ins[$key]['htmlBody'] .= '<div class="text-center"><b>'.$kitch[$key]['kitchen_name'].'</b></div>';
						...
						$row[$key][] = array('print_type' => 'receipt');
						$row[$key][] = array('type' => 'header', 'title' => 'No : '.$this->getPrintNuber());
						$row[$key][] = array('type' => 'header', 'title' => 'Mutfak Fisi');
						$row[$key][] = array('type' => 'header', 'title' => $this->getTableName($post['table_id']));
						//$row[$key][] = array('type' => 'header', 'title' => $_SESSION['user_name']);
						$row[$key][] = array('type' => 'header', 'title' => date('d.m.Y H:i', time()));
						$row[$key][] = array('type' => 'header', 'title' => $kitch[$key]['kitchen_name']);
						foreach($val as $vv){
							$row[$key][] = array('type' => 'line', 'title' => '------------------------------------------------' );
							$row[$key][] = array('type' => 'kitchen', 'title' => $vv['pro_name'],  'value' => $vv['qty'] );
						}
						
						foreach($val as $vv){
							$ins[$key]['htmlBody'] .= '<div class="tr f30 roww1"><div class="w90"><b>'.$vv['pro_name'].'</b></div><div class="w10 text-right"><b>'.$vv['qty'].'</b></div><div class="w33"></div><div class="clr"></div></div>';
						}
					...
					foreach($val as $kk => $vv){
						...
						$ins[$key]['jsonArray'] =  json_encode($row[$key], JSON_UNESCAPED_UNICODE);
	...
	public function getPrintNuber(){
		$check = $this->db->select('id')
			->order_by('id', 'DESC')
			->limit(1)
			->get('aaa_print_table')->row_array();
		return $check['id']+1;
	}
	...
	public function send_to_kitchen(){
		...
		$pro = $this->db->select('pro_id,pro_name,description2,qty')
			->where('id', $id)
			->get('order_details_table')->row_array();
		
		$pro['description2'] = str_replace("(Beklet)","",$pro['description2']);
		...
		//debug($kitch);
			if($kitch != 0){
				...
				$kitchen[$kitchenId]['pro_id'] = $pro_id;
				$kitchen[$kitchenId]['pro_name'] = $pro['pro_name'].' '.$pro['description2'];
				$kitchen[$kitchenId]['qty'] = $pro['qty'];
			}
		
		$this->db->update("order_details_table", array("description2" => $pro['description2']), array("id" => $id));
		...
		//debug($kitchen);
			foreach( $kitchen as $key => $val ){
				...
				//$row[$key][] = array('type' => 'header', 'title' => $_SESSION['user_name']);
				$row[$key][] = array('type' => 'header', 'title' => date('d.m.Y H:i', time()));
				$row[$key][] = array('type' => 'header', 'title' => $kitch['kitchen_name']);
				
					$row[$key][] = array('type' => 'line', 'title' => '------------------------------------------------' );
					$row[$key][] = array('type' => 'kitchen', 'title' => $val['pro_name'],  'value' => $val['qty'] );
					$row[$key][] = array('type' => 'line', 'title' => '------------------------------------------------' );
					$row[$key][] = array('type' => 'line', 'title' => ' ' );
					$row[$key][] = array('type' => 'line', 'title' => ' ' );
	...
	public function getCancels(){
		$list = $this->db->select('orders_table.order_insert_time, order_details_table.description, user_log_report.user_id, order_details_table.qty, order_details_table.pro_id, order_details_table.porsion_id, porsions_table.porsion_price')
			->join('orders_table', 'order_details_table.order_id = orders_table.order_id', 'left')
			->join('user_log_report', 'order_details_table.order_id = user_log_report.order_id', 'left')
			->join('porsions_table', 'order_details_table.porsion_id = porsions_table.id', 'left')
			->where('order_details_table.description', 'İptal')
			->get('order_details_table')->result_array();
	...
	public function getPrinterType($prName){
		$printer = $this->db->select('*')
			->where('printer_ip', $prName)
			->get('aaa_printers_table')->row_array();
		return $printer['printer_type'];
	}
	...
	public function qty_change(){
		...
		$order_id = $post['order_id'];
		$table_id = $this->db->select('table_id')->where('order_id', $order_id)->get('orders_table')->row_array()['table_id'];
		
		$order_row = $this->db->select('*')
			->where('id', $post['id'])
			->get('order_details_table')->row_array();
		
		if( $order_row['qty'] > $post['cancelQty'] ){
			$whr['id'] = $post['id'];
			$upd['qty'] = $order_row['qty'] - $post['cancelQty'];
			$upd['total_price'] = $order_row['price']*$upd['qty'];
				$this->db->update("order_details_table", $upd, $whr);
			
			if($this->db->affected_rows() > 0){
				
				$ins['order_id'] = $order_row['order_id'];
				$ins['porsion_id'] = $order_row['porsion_id'];
				$ins['pro_id'] = $order_row['pro_id'];
				$ins['pro_name'] = $order_row['pro_name'];
				$ins['description'] = $post['type'];
				$ins['qty'] = $post['cancelQty'];
				$ins['price'] = '0.00';
				$ins['total_price'] = '0.00';
				$ins['printed'] = 1;
				...
				if($this->db->affected_rows() > 0){
					...
					if($post['type'] == 'İptal'){
						...
						$ins1['printerName'] = $this->findKitchen($order_row['pro_id'])['kitchen_ip']; 
						if(!empty($ins1['printerName'])){
							$kitchen_name = $this->findKitchen($order_row['pro_id'])['kitchen_name']; 
							$ins1['printer_type'] = $this->getPrinterType($ins1['printerName']); 
							
							$row[] = array('print_type' => 'receipt');
							$row[] = array('type' => 'header', 'title' => $this->getTableName($table_id));
							//$row[] = array('type' => 'header', 'title' => $_SESSION['user_name']);
							$row[] = array('type' => 'header', 'title' => date('d.m.Y H:i', time()));
							$row[] = array('type' => 'header', 'title' => $kitchen_name);
							$row[] = array('type' => 'line', 'title' => '------------------------------------------------');
							$row[] = array('type' => 'cancel', 'title' => $order_row['pro_name'] );
							$row[] = array('type' => 'cancel', 'title' => $post['cancelQty'].' x (IPTAL)' );
							
							$ins1['jsonArray'] =  json_encode($row, JSON_UNESCAPED_UNICODE);
							$ins1['imgName'] = date('d-m-Y-His', time()).uniqid().'.png';
							$ins1['insertTime'] = time();
							
							$this->db->insert('aaa_print_table',$ins1);
						}

						$report_log['user_id'] = $_SESSION['user_id'];
						$report_log['order_id'] = $order_row['order_id'];
						$report_log['process'] = 'cancel';
						$report_log['description'] = "";
						$report_log['first_price'] = ($order_row['price']*$post['cancelQty']);
						$report_log['last_price'] = "0";
						$report_log['pro_id'] = $order_row['pro_id'];
						$report_log['qty'] = $post['cancelQty'];
		...
		if( $order_row['qty'] == $post['cancelQty'] ){
			...
			$upd['description'] = $post['type'];
			//$upd['qty'] = 0;
			$upd['price'] = '0.00';
			$upd['total_price'] = '0.00';
			$upd['printed'] = 1;
				$this->db->update("order_details_table", $upd, $whr);
	...
	public function order_price_update($order_id){
		$total = $this->db->select('SUM(total_price) total_price')
			->where('order_id', $order_id)
			->get('order_details_table')->row_array();
		debug($total);
	}
		...
		if(!empty($kitchen)){
			...
			if( $post['cancel'] == '1' ){
				...
				foreach( $kitchen as $key => $val ){
					...
					$ins[$key]['jsonArray'] =  json_encode($json, JSON_UNESCAPED_UNICODE);
					$ins[$key]['imgName'] = date('d-m-Y-His', time()).uniqid().'.png';
					$ins[$key]['insertTime'] = time();
						$this->db->insert('aaa_print_table',$ins[$key]);
		...
		if(!empty($post['pro_id'])){
			...
			//debug($kitchen);
					foreach( $kitchen as $key => $val ){
						...
						$row[$key][] = array('print_type' => 'receipt');
						$row[$key][] = array('type' => 'header', 'title' => 'Mutfak Fisi');
						$row[$key][] = array('type' => 'header', 'title' => 'Paket Siparis');
						$row[$key][] = array('type' => 'header', 'title' => $cust['full_name']);
						//$row[$key][] = array('type' => 'header', 'title' => $_SESSION['user_name']);
						$row[$key][] = array('type' => 'header', 'title' => date('d.m.Y H:i', time()));
						foreach($val as $vv){
							$row[$key][] = array('type' => 'line', 'title' => '------------------------------------------------' );
	...
	public function payment_post(){
		...
		if($order['rest_price'] >= $post['amount']){
			...
			if($post['payment_type'] == 'discount'){
				...
				$upd['discount_price'] = ($post['amount']);
				$upd['paid_price'] = (($order['paid_price']-$order['discount_price']) + $post['amount']);
				$upd['rest_price'] = $order['total_price'] - $upd['paid_price'];
				...
				if($this->db->affected_rows() > 0){ 
					...
					$whr = array('payment_type' => 'discount', 'order_id' => $post['order_id']);
					$this->db->delete('order_payments_table', $whr);
					...
					if($upd['rest_price'] == '0'){
						$this->print_order($post['order_id']);
				...
				if($this->db->affected_rows() > 0){ 
					...
					if($post['payment_type'] == 'point'){
						$ins11['point'] = number_format( ($post['amount']), 2 );
						$ins11['customer_id'] = $post['customer_id'];
						$ins11['order_id'] = $post['order_id'];
						$ins11['type'] = 'minus';
						$ins11['insert_time'] = time();
							$this->db->insert("customer_point_table",$ins11);
	...
	public function print_order($order_id){
		...
		$order = $this->db->select('*')
			->where('order_id', $order_id)
			->join('products_table2', 'order_details_table.pro_id = products_table2.pro_id', 'left')
			->where('total_price > 0')
			->get('order_details_table')->result_array();
		...
		print_rows($order,$order_id, "marifet");
	...
	public function discount_cancel(){
		...
		if($post['order_id'] > 0){
			$upd['discount_price'] = '0.00';
			$upd['paid_price'] = $order['paid_price'] - $order['discount_price'];
			$upd['rest_price'] = $order['rest_price'] + $order['discount_price'];
	...
	public function add_customer_to_phone_order(){
		...
		//debug($post);
				...
				if(empty($check)){
						...
						$ins['seo_full_name'] = tr_seo_converter($post['full_name']);
						$ins['phone'] = $post['phone'];
						$ins['phone2'] = str_replace('-','',$post['phone']);
	...
	public function search_customers(){
		...
		if(!empty($post['name'])){
			...
			$data['phone_results'] = $this->db->select('*')
				->like('phone2', $post['name'], 'both')
				->limit(5)
				->get('customers_table')->result_array();
			
			$data['barcode_results'] = $this->db->select('*')
				->like('customer_barcode', $post['name'], 'both')
				->limit(5)
				->get('customers_table')->result_array();
		...
		$data['results'] = array_merge($data['results'],$data['barcode_results']);
	...
	public function getCustomerDiscount($customer_id){
		...
		$cust = $this->db->select('customer_group_id')
			->where('customer_id', $customer_id)
			->get('customers_table')->row_array();
		
		$c = $this->db->select('*')
			->where('id', $cust['customer_group_id'])
			->get('customer_groups_table')->row_array();
		
		return $c['group_point'];
		
	}
	
	public function checkRecipe($porsion_id){
		$recipes = array();
		$recipe = $this->db->select('*')
			->where('porsion_id', $porsion_id)
			->get('recipes_table')->result_array();
	...
	public function saleRecipe($products){
	...
	public function getRecipe($c_pro_id,$qty,$order_id){
		
		$uniqid  = uniqid();
		$recipe[$uniqid] = $this->db->select('*')
			->where('porsion_id', $c_pro_id)
			->get('recipes_table')->result_array();
		
		
		
		if(!empty($recipe[$uniqid])){
			foreach($recipe[$uniqid] as $key => $val){
				$recipe[$uniqid][$key]['qty2'] = $qty;
				$recipe[$uniqid][$key]['order_id'] = $order_id;
				 $this->getRecipe($val['c_pro_id'],($val['qty'] * $qty), $order_id);
			}
			
			
			$_SESSION['recipes'][$c_pro_id] = $recipe[$uniqid];
	...
	public function close_table(){
		...
		$last_order = $this->db->select('order_id')
			->order_by('order_id', 'DESC')
			->limit(1)
			->get('orders_table')->row_array();
		
		$table_id = '1';
		$order_id = $last_order['order_id'];//$post['order_id'];
		...
		foreach($products as $key => $val){
			...
			if(!empty($products[$key]['recipe'])){
				$this->getRecipe($val['porsion_id'], $val['qty'], $order_id);
			}else{
				$ins['order_id'] = $order_id;
				$ins['pro_id'] = $val['pro_id'];
				$ins['qty'] = $val['qty'];
		...
		/* if(!empty($_SESSION['recipes'])){
			foreach($_SESSION['recipes'] as $key => $val){
				foreach($val as $kk => $vv){
					$recipes[] = $vv;
				}
			}
		...
		foreach($recipes as $kk => $vv){
			$ins['order_id'] = $order_id;
			$ins['porsion_id'] = $vv['porsion_id'];
			$ins['pro_id'] = $vv['c_pro_id'];
			$ins['qty'] = ($vv['qty'] * $vv['qty2']);
			$ins['unit'] = $vv['unit'];
			$ins['insert_time'] = time();
				$this->db->insert('sales_table', $ins);
		}
	...
	public function calcPoint($customer_id){
		
		$plus = $this->db->select('SUM( point ) total')
			->where('customer_id', $customer_id)
			->where('type', 'plus')
			->get('customer_point_table')->row_array();
		
		$minus = $this->db->select('SUM( point ) total')
			->where('customer_id', $customer_id)
			->where('type', 'minus')
			->get('customer_point_table')->row_array();
			
			
		$total = number_format(($plus['total'] - $minus['total']),2);
	...
	public function reload_orders($kid){
		...
		$data['orders'] = $this->db->select('order_id, order_insert_time, orderType, full_name, table_name, order_note')
			->join('tables_table', 'orders_table.table_id = tables_table.table_id', 'left')
			->join('customers_table', 'orders_table.customer_id = customers_table.customer_id', 'left')
			->where('rest_price > 0')
			->get('orders_table')->result_array();
	...
	public function reload_orders2(){
		...
		$data['orders'] = $this->db->select('order_id,order_insert_time,table_name,order_note')
			->join('tables_table', 'orders_table.table_id = tables_table.table_id', 'left')
			->where('rest_price > 0')
			->get('orders_table')->result_array();
		//debug($data['orders']);
	...
	public function customer_groups(){
		$data['groups'] = $this->db->select('*')
			->get('customer_groups_table')->result_array();
			
		$data['mm'] = '161';	
		$data['mt'] = '161_3';
			
		$this->load->view("customer_groups_view", $data);
	}
	
	public function add_group_post(){
		$post = $this->input->post();
		
		$ins['group_name'] = $post['group_name'];
		$ins['group_discount'] = $post['group_discount'];
		$ins['insert_time'] = time();
		
		$this->db->insert("customer_groups_table", $ins);
	...
	public function customers(){
		$data['customers'] = $this->db->select('*')
			...
			->join('customer_groups_table', 'customers_table.customer_group_id = customer_groups_table.id', 'left')
			->get('customers_table')->result_array();
	...
	public function group_update_post(){
		...
		foreach($post['ids'] as $key => $val){
			$this->db->update("customers_table", array("customer_group_id" => $post['group_id']), array("customer_id" => $val));
		}
	...
	public function customer_group_details($id){
		$data['group'] = $this->db->select('*')
			->where('id', $id)
			->get('customer_groups_table')->row_array();
		$this->load->view('customer_group_details_view', $data);
	}
	
	public function customer_group_update_post(){
		$post = $this->input->post();
		
		$whr['id'] = $post['id'];
		$upd['group_name'] = $post['group_name'];
		$upd['group_discount'] = $post['group_discount'];
		$upd['group_point'] = $post['group_point'];
			$this->db->update("customer_groups_table", $upd, $whr);
		redirect($_SERVER['HTTP_REFERER'] );
	}
	...
	public function customer_update_post(){
		...
		$upd['email'] = $post['email'];
		$upd['phone'] = $post['phone'];
		$upd['customer_group_id'] = $post['customer_group_id'];
		$upd['customer_barcode'] = $post['customer_barcode'];
			$this->db->update('customers_table', $upd, $whr);
	...
	public function reports_post(){
		...
		if($type == 'month_simple'){
			$this->month_details_simple($fdate, $ldate);
		}
	...
	public function get_order_details_logs(){
		...
		$data['settings'] = $this->db->select('*')
			->where('id', 1)
			->get('settings_table')->row_array();
		$data['details'] = $this->db->select('order_details_table.pro_name, order_details_table.qty, order_details_table.price')
			->where('order_details_table.order_id', $post['order_id'])
			->get('order_details_table')->result_array();
		$data['list'] = $data['details'];
		$data['details']['ord'] = $this->db->select('*')
			->join('tables_table', 'orders_table.table_id = tables_table.table_id', 'left')
	...
	public function get_cat_or_products($cat_id){
		...
		if(!empty($data['sub_cat_list'])){
			...
			$data['pro_list'] = $this->db->select('pro_name, products_table2.pro_id, cat_id, pro_bg, proType')
				->join('products_table2', 'menu_cat_product_table.pro_id = products_table2.pro_id', 'left')
	...
	public function update_printer_post(){
		...
		$upd['printer_type'] = $post['printer_type'];
